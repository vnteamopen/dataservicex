...
func buildPostgresDataSource(host, database, username, password string, port int) string {
...
func TestCRUD(t *testing.T) {
	dataSource := buildPostgresDataSource("localhost", "dataservicex", "dataservicex", "dataservicex", 5432)
	...
	createdPerson2, err := dataService.Create(context.Background(), Person{
		...
		Age:       26,
		Height:    180.5,
		CreatedAt: sql.NullTime{Valid: true, Time: createdAt2},
	})
	assert.NoError(t, err, "create person")
